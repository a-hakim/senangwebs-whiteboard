...
export const ContextMenuMixin = {
  ...
  updateContextMenuState() {
    ...
    const hasEditableSelection = Array.from(this.selectedElements).some(
      (el) =>
        el.type === "text" ||
        el.type === "website" ||
        el.type === "image" ||
    ...
    const hasLockedSelection = Array.from(this.selectedElements).some(
      (el) => el.locked
    );
    const hasUnlockedSelection = Array.from(this.selectedElements).some(
      (el) => !el.locked
    );
    ...
    const duplicateItem = this.contextMenu.querySelector(
      '[data-action="duplicate"]'
    );
    const groupItem = this.contextMenu.querySelector('[data-action="group"]');
    const ungroupItem = this.contextMenu.querySelector(
      '[data-action="ungroup"]'
    );
    ...
    const bringToFrontItem = this.contextMenu.querySelector(
      '[data-action="bring-to-front"]'
    );
    const sendToBackItem = this.contextMenu.querySelector(
    ...
    if (ungroupItem) {
      ...
      ungroupItem.style.cursor = hasGroupedSelection
        ? "pointer"
        : "not-allowed";
    }
  ...
  addContextMenuItem(item) {
    ...
    const existingItem = this.contextMenu.querySelector(
      `[data-action="${item.id}"]`
    );
    if (existingItem) {
    ...
    if (item.position === "top") {
      ...
      const targetItem = this.contextMenu.querySelector(
        `[data-action="${item.position}"]`
      );
